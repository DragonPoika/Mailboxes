...
execute as @e[type=interaction,tag=mailbox_interaction,distance=..6] if data entity @s interaction at @s run data modify entity @e[type=marker,tag=mailbox_storage,limit=1,sort=nearest] data.Mail append from entity @a[tag=mailbox.this,distance=..6,limit=1,sort=nearest] SelectedItem
