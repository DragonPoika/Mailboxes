...
execute as @e[type=interaction,tag=mailbox_interaction,scores={mailbox.settings=0}] at @s unless block ~ ~ ~ minecraft:air run function mailboxes:hit
...
execute as @a[tag=!mailbox.notifi] at @s if entity @e[type=marker,tag=mailbox_storage,distance=..4,scores={mailbox.itemcount=1..}] run advancement grant @s only mailboxes:get_mail
